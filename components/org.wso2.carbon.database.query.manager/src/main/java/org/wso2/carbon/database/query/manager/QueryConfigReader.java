...
public class QueryConfigReader {
    private DatabaseQueryConfig baseQueryConfigMap;
    private DatabaseMetaData currentQueryKey;

    public QueryConfigReader(DatabaseQueryConfig baseQueryConfig, DatabaseMetaData currentQueryKey) {
    ...
    public String getQueryTemplate(String queryName) {
        ...
        if (baseQueryConfigMap != null && currentQueryKey != null) {
            template = baseQueryConfigMap.getQueries(currentQueryKey).get(queryName);
        }
        return template;
    }
}
